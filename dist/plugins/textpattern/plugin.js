...
var textpattern = (function () {
  ...
  var $_7p9f4vqljegz3zdk = { getPatterns: getPatterns };
  ...
  var patternFromRng = function (patterns, rng, space) {
    ...
    var endPattern = $_9bel0zqtjegz3ze4.findEndPattern(patterns, text, rng.startOffset, delta);
    ...
    var startPattern = $_9bel0zqtjegz3ze4.findPattern(patterns, patternRng.toString());
  ...
  var applyBlockFormat = function (editor, patterns) {
    ...
    if (textBlockElm) {
      ...
      if (firstTextNode) {
        pattern = $_9bel0zqtjegz3ze4.findPattern(patterns, firstTextNode.data);
        if (!pattern) {
          return;
        }
  ...
  function handleEnter(editor, patterns) {
    ...
    wrappedTextNode = $_5px1yeqqjegz3zdy.applyInlineFormatEnter(editor, patterns);
    if (wrappedTextNode) {
    ...
    $_5px1yeqqjegz3zdy.applyBlockFormat(editor, patterns);
  }
  function handleInlineKey(editor, patterns) {
    var wrappedTextNode, lastChar, lastCharNode, rng, dom;
    wrappedTextNode = $_5px1yeqqjegz3zdy.applyInlineFormatSpace(editor, patterns);
    if (wrappedTextNode) {
      dom = editor.dom;
  ...
  var setup = function (editor, patternsState) {
    ...
    editor.on('keydown', function (e) {
      if (e.keyCode === 13 && !VK.modifierPressed(e)) {
        $_jkcdaqpjegz3zdr.handleEnter(editor, patternsState.get());
      }
    }, true);
    editor.on('keyup', function (e) {
      if ($_jkcdaqpjegz3zdr.checkKeyCode(keyCodes, e)) {
        $_jkcdaqpjegz3zdr.handleInlineKey(editor, patternsState.get());
      }
    });
    editor.on('keypress', function (e) {
      if ($_jkcdaqpjegz3zdr.checkCharCode(charCodes, e)) {
  ...
  var $_41m2viqmjegz3zdm = { setup: setup };

  PluginManager.add('textpattern', function (editor) {
    var patternsState = Cell($_7p9f4vqljegz3zdk.getPatterns(editor.settings));
    $_41m2viqmjegz3zdm.setup(editor, patternsState);
    return $_7z0v4oqkjegz3zdj.get(patternsState);
  });
  function Plugin () {
  }
