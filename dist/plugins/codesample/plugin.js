...
var codesample = (function () {
  ...
  var getSelectedCodeSample = function (editor) {
    ...
    if ($_azscii9njegz3vu8.isCodeSample(node)) {
      return node;
    }
    return null;
  };
  ...
  var getLanguages$1 = function (editor) {
    ...
    var customLanguages = $_d9gaxc9jjegz3vsh.getLanguages(editor);
    return customLanguages ? customLanguages : defaultLanguages;
  };
  var getCurrentLanguage = function (editor) {
    var matches;
    var node = $_5ay10b9ljegz3vsk.getSelectedCodeSample(editor);
    if (node) {
  ...
  var $_fn53zo9ojegz3vua = {
    getLanguages: getLanguages$1,
    getCurrentLanguage: getCurrentLanguage
  };

  var $_3pq0lv9ijegz3vsg = {
    open: function (editor) {
      var minWidth = $_d9gaxc9jjegz3vsh.getDialogMinWidth(editor);
      var minHeight = $_d9gaxc9jjegz3vsh.getDialogMinHeight(editor);
      var currentLanguage = $_fn53zo9ojegz3vua.getCurrentLanguage(editor);
      var currentLanguages = $_fn53zo9ojegz3vua.getLanguages(editor);
      var currentCode = $_5ay10b9ljegz3vsk.getCurrentCode(editor);
      editor.windowManager.open({
        ...
        onSubmit: function (e) {
          $_5ay10b9ljegz3vsk.insertCodeSample(editor, e.data.language, e.data.code);
        }
      });
    }
  };
  ...
  var register = function (editor) {
    editor.addCommand('codesample', function () {
      var node = editor.selection.getNode();
      if (editor.selection.isCollapsed() || $_azscii9njegz3vu8.isCodeSample(node)) {
        $_3pq0lv9ijegz3vsg.open(editor);
      } else {
        editor.formatter.toggle('code');
      }
    });
  };
  var $_62l8l09hjegz3vse = { register: register };

  var setup = function (editor) {
    var $ = editor.$;
    editor.on('PreProcess', function (e) {
      $('pre[contenteditable=false]', e.node).filter($_azscii9njegz3vu8.trimArg($_azscii9njegz3vu8.isCodeSample)).each(function (idx, elm) {
    ...
    editor.on('SetContent', function () {
      var unprocessedCodeSamples = $('pre').filter($_azscii9njegz3vu8.trimArg($_azscii9njegz3vu8.isCodeSample)).filter(function (idx, elm) {
  ...
  var $_3bvsq79pjegz3vuc = { setup: setup };

  var loadCss = function (editor, pluginUrl, addedInlineCss, addedCss) {
    var linkElm;
    var contentCss = $_d9gaxc9jjegz3vsh.getContentCss(editor);
  ...
  var $_ekk0469qjegz3vuf = { loadCss: loadCss };
  ...
  var $_2qrga29rjegz3vui = { register: register$1 };

  var addedInlineCss = Cell(false);
  PluginManager.add('codesample', function (editor, pluginUrl) {
    var addedCss = Cell(false);
    $_3bvsq79pjegz3vuc.setup(editor);
    $_2qrga29rjegz3vui.register(editor);
    $_62l8l09hjegz3vse.register(editor);
    editor.on('init', function () {
      $_ekk0469qjegz3vuf.loadCss(editor, pluginUrl, addedInlineCss, addedCss);
    });
    editor.on('dblclick', function (ev) {
      if ($_azscii9njegz3vu8.isCodeSample(ev.target)) {
        $_3pq0lv9ijegz3vsg.open(editor);
      }
    });
  });
